...
from .data import build_user_and_gold, cqa_user, earliest_correct_step, is_correct, load_examples, load_exemplar_pool, parse_pred
from .extraction import CONF_FEATURES, extract_compact, extract_confidence, write_multi_csv
...
def recover_confidence(recs, classifier_keys, model_key, dataset_name, out_name, drive_dir,
                       cfg=None, checkpoint_every=200):
    """Second deterministic pass that recovers per-token confidence features
    (extract_confidence) for every row already collected by run_multi, without needing to
    re-run the original (hidden-state-carrying) generation. Checkpoints to drive_dir every
    `checkpoint_every` rows and resumes automatically if interrupted/disconnected.

    Row order MUST match build_feature_table's / collect_rows's row order exactly (original
    first, then each classifier_key in order) for resume/alignment to hold.
    """
    cfg = dict(cfg or COMPACT_CFG)
    spec = MODELS[model_key]
    conf_path = os.path.join(drive_dir, f"{spec['short']}__{dataset_name}__{out_name}_confidence.pt")
    conf_local = (f"/content/{spec['short']}__{dataset_name}__{out_name}_confidence.temp.pt"
                  if os.path.isdir("/content") else conf_path + ".temp")

    tok, model, _ = load_model(model_key)
    stop_ids = stop_token_ids(tok, model)
    think = cfg["enable_thinking"] if model_key.startswith("qwen3") else None
    examples_ds, _ = load_examples(dataset_name, len(recs))

    def _row_prompt(idx, source, tf_question):
        ex = examples_ds[idx]
        q = ex["question"] if source == "original" else tf_question
        return cqa_user(q, ex["choices"])

    pbar_rows = []
    for r in recs:
        o = r["original"]
        if o["metrics"].numel() > 0:
            pbar_rows.append((r["idx"], "original", None, o["generated"]))
        for vkey in classifier_keys:
            ...
            for s in v["samples"]:
                if s["metrics"].numel() == 0:
                    continue
                pbar_rows.append((r["idx"], vkey, s.get("tf_question"), s["generated"]))

    if os.path.exists(conf_path):
        saved = torch.load(conf_path, weights_only=False)
        confidence_by_row, mismatches = saved["confidence_by_row"], saved["mismatches"]
        print(f"Resuming: {len(confidence_by_row)}/{len(pbar_rows)} rows already saved in {conf_path}")
    else:
        confidence_by_row, mismatches = [], 0

    def _flush():
        torch.save({"confidence_by_row": confidence_by_row, "mismatches": mismatches}, conf_local)
        shutil.copy2(conf_local, conf_path)

    start_i = len(confidence_by_row)
    print(f"Recovering confidence features for rows {start_i}..{len(pbar_rows)-1}")
    for i in tqdm(range(start_i, len(pbar_rows))):
        idx, source, tf_q, stored_text = pbar_rows[i]
        prompt = format_prompt(tok, _row_prompt(idx, source, tf_q), think)
        r_c = extract_confidence(tok, model, prompt, cfg, stop_ids)
        if r_c["text"] != stored_text:
            mismatches += 1
        confidence_by_row.append(r_c)
    ...
    _flush()
    print(f"\nDone -> {conf_path}")
    print(f"{mismatches}/{len(pbar_rows)} regenerations mismatched stored text "
          f"({mismatches/len(pbar_rows):.1%}) -- should be ~0% since inference is deterministic.")
    ...
    Xconf = np.array([[c[f] for f in CONF_FEATURES] for c in confidence_by_row], dtype=np.float32)
    print(f"confidence feature matrix: {Xconf.shape}")
    return confidence_by_row, Xconf, conf_path
